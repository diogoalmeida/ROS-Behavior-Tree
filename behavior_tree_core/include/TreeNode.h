...
// Enumerates the states every node can be in after execution during a
// particular
// time step:
// - "Success" indicates that the node has completed running during this time
// step;
// - "Failure" indicates that the node has determined it will not be able to
// complete
//   its task;
// - "Running" indicates that the node has successfully moved forward during
// this
...
// Enumerates the options for when a parallel node is considered to have
// succeeded:
// - "SucceedOnOne" indicates that the node will return success as soon as one
//   of its children succeeds;
// - "SucceedOnAll" indicates that all of the node's children must succeed
// before
//   it returns success.
enum SuccessPolicy { SucceedOnOne, SucceedOnAll };

// If "FailOnOne" and "SucceedOnOne" are both active and are both trigerred in
// the
