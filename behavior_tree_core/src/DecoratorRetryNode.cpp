...
DecoratorRetryNode::DecoratorRetryNode(std::string Name, unsigned int NTries)
      ...
      // for (i = 0; i<M; i++)
      {
        ...
        if (ChildStates[0] == Success) {
          ...
          if (ChildStates[0] == Failure && TryIndx_ < NTries_) {
            // 3.1) the node state is equal to running since I am rerunning the
            // child
            ...
            // 3.2) state reset;
            WriteState(Idle);
            std::cout << Name << " returning " << ChildStates[0] << "!"
                      << std::endl;
          }
        }
      }
