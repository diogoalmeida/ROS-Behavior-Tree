...
DecoratorNegationNode::DecoratorNegationNode(std::string Name)
  ...
  while (true) {
    ...
    if (ReadState() != Halted) {
      ...
      if (ChildStates[0] == Success) {
        // 3.1) the node state is equal to failure since I am negating the
        // status
        ...
        // 4.1) the node state is equal to success since I am negating the
        // status
      ...
      if (ChildNodes[0]->Type != Action && ChildStates[0] == Running) {
        ...
        std::cout << Name << " halting child  "
                  << "!" << std::endl;
      } else if (ChildNodes[0]->Type == Action &&
                 ChildNodes[0]->ReadState() == Running) {
        std::cout << Name << " trying halting child  "
                  << "..." << std::endl;
        ...
        if (ChildNodes[0]->Halt() == false) {
          ...
          std::cout << Name << " halting of child  "
                    << " failed!" << std::endl;
        }
