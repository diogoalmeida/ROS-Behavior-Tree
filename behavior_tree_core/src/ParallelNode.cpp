...
void ParallelNode::Exec() {
  ...
  if (N == std::numeric_limits<unsigned int>::max()) {
    throw BehaviorTreeException("'" + Name + "' has no valid N threashold set. "
                                             "You should set it before ticking "
                                             "the node.");
  }
  ...
  while (true) {
    ...
    if (ReadState() != Halted) {
      ...
      if (StateUpdate == true) {
        ...
        if (ReadState() == Success || ReadState() == Failure) {
          ...
          for (i = 0; i < M; i++) {
            ...
            std::cout << Name << " trying halting (action) child number " << i
                      << "..." << std::endl;
            ...
            if (ChildNodes[i]->Halt() == false) {
              ...
              std::cout << Name << " halting of child number " << i
                        << " failed!" << std::endl;
            } else {
              std::cout << Name << " halting of child number " << i
                        << " succedeed!" << std::endl;
          ...
          // return Running.
          std::cout << Name << " ticking the remaining children... "
                    << std::endl;
          ...
          for (i = 0; i < M; i++) {
            ...
            if (ChildStates[i] == Running) {
              std::cout << Name << " halting (control) child number " << i
                        << "..." << std::endl;
          ...
          std::cout
              << Name
              << " ticking the remaining children and ignoring their state... "
              << std::endl;
      ...
      std::cout << Name << " doesn't know yet what to return, ticking the "
                           "remaining children..." << std::endl;
      ...
      if (StateUpdate == true && ReadState() != Running) {
        ...
        for (i = 0; i < M; i++) {
          ...
          if (ChildNodes[i]->Type == Action) {
            ...
            std::cout << Name << " halting child number " << i << "!"
                      << std::endl;
          }
      ...
      /*       for(i=0; i<M; i++)
         ...
         << std::endl;
                 ...
                 else if (ChildNodes[i]->Type == Action &&
         ChildNodes[i]->ReadState() == Running)
                 {
                     std::cout << Name << " trying halting child number " << i
         << "..." << std::endl;
