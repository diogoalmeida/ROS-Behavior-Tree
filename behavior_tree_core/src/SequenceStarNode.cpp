...
SequenceStarNode::SequenceStarNode(std::string Name)
    ...
    if (ReadState() != Halted) {
      ...
      while (i < M) {
        ...
        if (ChildStates[i] != Success) {
          ...
          break;
        } else if (ChildStates[i] == Success) // If the child i returns success,
                                              // the sequence star node can tick
                                              // the next child
        {
          i++;
      ...
      if (i == M) {
        ...
        SetNodeState(Success); // Final State of rhe selector node. Child index
                               // reinitialized
