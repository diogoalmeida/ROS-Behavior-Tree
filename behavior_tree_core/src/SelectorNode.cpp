...
*/
      ...
      /* for(int j=0; j<M; j++)
       ...
       std::endl;
           ...
           else if (ChildNodes[j]->Type == Action && ChildNodes[j]->ReadState()
       == Running)
           {
               std::cout << Name << " trying halting child number " << j <<
       "..." << std::endl;
               ...
               if (ChildNodes[j]->Halt() == false)
               {
                   // this means that, before this node could set its child
       state
                   // to "Halted", the child had already written the action
       outcome;
                   // sync with him ignoring its state;
                   ChildNodes[j]->Semaphore.Signal();
