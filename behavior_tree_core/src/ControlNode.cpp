...
void ControlNode::AddChild(TreeNode *Child) {
  ...
  for (unsigned int i = 0; i < ChildNodes.size(); i++) {
    if (ChildNodes[i] == Child) {
      throw BehaviorTreeException("'" + Child->Name + "' is already a '" +
                                  Name + "' child.");
    }
  }
...
unsigned int ControlNode::GetChildrenNumber() { return ChildNodes.size(); }
...
std::vector<TreeNode *> ControlNode::GetChildren() { return ChildNodes; }
  ...
  for (unsigned int j = i; j < ChildNodes.size(); j++) {
    if (ChildNodes[j]->Type != Action && ChildStates[j] == Running) {
      ...
      if (ChildNodes[j]->Halt() == false) {
        ...
        std::cout << Name << " halting of child number " << j << " failed!"
                  << std::endl;
      }

      std::cout << Name << " halting of child number " << j << " succedeed!"
                << std::endl;
    } else if (ChildNodes[j]->Type == Action &&
